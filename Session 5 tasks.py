...
import numpy as np
a=([1,1,1],[2,2,2,],[3,3,3],[1,2,3],[1,2,3],[1,2,3])
A=np.array(a)                                         #to create pattern of the array
print(A)


import numpy as np
data=np.array([[1,2,3,4,5],[4,5,6,7,8]]) 
new_array=[tuple(row) for row in data]     #to remove duplicates I used this
uniques=np.unique(data)                     # to sort the unique (non duplicated) list
print(uniques)  
